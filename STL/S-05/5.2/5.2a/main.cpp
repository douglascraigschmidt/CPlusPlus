#include <iostream>
#include <utility>

#include "type_wrapper.h"
#include "point.h"
using namespace std;

/**
 * This program demonstrates several usage of the STL pair template.
 */
int main() {
  // Instantiate an object of the point class to make a point.
  point<int, int> my_point(100, 200);
    
  cout << "x = " << my_point.x << "\ny = " << my_point.y << endl;
    
  // Create several other more interesting "points".
  point<string, string> book_chapter_1("Chapter1",
                                     "Meet the C++ STL");
  point<string, void *> debug("mynum",
                            reinterpret_cast<void *>(0x410928a8));

  point<type_wrapper<string>, type_wrapper<string>>
    book_chapter_2(std::move(string("Chapter2")),
                   std::move(string("Meet the C++ STL associative containers")));

  cout << "book_chapter_2.x: "
       << string(book_chapter_2.x)
       << "\nbook_chapter_2.y: "
       << string(book_chapter_2.y)
       << endl;

  return 0;
}
